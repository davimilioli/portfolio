        ...
        <div className="w-full h-full flex flex-col rounded-2xl border border-neutral-200 bg-white overflow-hidden shadow-sm my-6">
            <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-200 bg-neutral-50">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono font-semibold text-neutral-500 tracking-wide">
                        {label}
                    </span>
                </div>
                <button
                    ...
                    className="flex items-center gap-1 text-xs font-mono uppercase tracking-wide px-2 py-1 rounded-md bg-neutral-100 text-neutral-500 hover:bg-neutral-200 hover:text-neutral-700 transition-colors cursor-pointer"
                >
                ...
                {steps.map((current, index) => {
                    ...
                    return (
                        <div key={index} className={`flex flex-col items-start gap-1 animate-message-in ${current.type === 'code' ? 'w-full' : ''}`}>
                            ...
                            {current.type === 'code' && (
                                <pre className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-xs leading-relaxed font-mono text-neutral-200 overflow-x-auto">
                                    {current.content}
                                </pre>
                            )}
                        </div>
